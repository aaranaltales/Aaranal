...
const OrderDetailsPage = () => {
  ...
  useEffect(() => {
    ...
    // after setting the order order.items will contain id and quantity of product so now i want to add product details we can filter it from allProducts

  }, [id, token]);

  useEffect(() => {
    if (order && allProducts.length > 0) {
      // check if items are already enriched (avoid infinite loop)
      const alreadyEnriched = order.items.every((item) => item._id);

      if (!alreadyEnriched) {
        const enrichedItems = order.items.map((item) => {
          const product = allProducts.find((p) => p._id === item.productId);
          if (!product) return item; // fallback in case product not found

          return {
            ...product,         // spread product fields (_id, name, description, etc.)
            quantity: item.quantity, // keep quantity from order
          };
        });
